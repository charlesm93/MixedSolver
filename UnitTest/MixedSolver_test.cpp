...
TEST(mix_solver, FK_vv)  {
  ...
  // Using multiple simulations (NSim) and multiple patients
  // (N_max) comes in handy if we want to measure CPU time. In this
  // file I only test the solver return the right solution and
  // Jacobian.
  ...
  // two simulations)
  expect_near_matrix_eq(J_num, J_mix, 5e-4);
}
